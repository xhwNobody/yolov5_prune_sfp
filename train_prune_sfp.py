...
#            attempt_download(weights)  # download if not found locally
        ...
        model = Model(nc=nc).to(device)  # create
    ...
    optimizer.add_param_group({'params': pg1, 'weight_decay': hyp['weight_decay']})  # add pg1 with weight_decay 'weight_decay': hyp['weight_decay']
    ...
    #print('@@@@@@@@@@@@@@@@@', 'True' if rank in [-1, 0] else 'False')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        m.model = model                             ####################################
        m.if_zero()                                 ####################################
        m.init_mask(0.7, 3, prune_indexnum, True)   ####################################
        m.do_mask()                                 ####################################
        m.if_zero()                                 ####################################
        model = m.model                             ####################################
...
#get_filter_codebook, convert2tensor, init_length, init_rate, init_mask, do_mask, if_zero
class Mask:
    ...
    def get_filter_codebook(self, weight_torch, compress_rate, length):
        ...
        if len(weight_torch.size()) == 4:    #########################################################
            filter_pruned_num = int(weight_torch.size()[0] * (1 - compress_rate)) + 1 #为了配合yolov5 small model
    ...
    def init_rate(self, layer_rate, layer_begin, layer_end):
        ...
        self.mask_index = []
        for index, item in enumerate(self.model.parameters()):
            if len(item.shape) > 1 and index >= layer_begin and index <= layer_end:  #if len(item.shape) > 1 and index >= layer_begin and index <= layer_end
                self.mask_index.append(index)
            #print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$', len(self.mask_index))

        for key in self.mask_index:
            self.compress_rate[key] = layer_rate

    def init_mask(self, layer_rate, layer_begin, layer_end, use_cuda):
        self.init_rate(layer_rate, layer_begin, layer_end)
        for index, item in enumerate(self.model.parameters()):
    ...
    def if_zero(self):#layer_begin, layer_end
        for index, item in enumerate(self.model.parameters()):
            if index in self.mask_index:
                a = item.data.view(self.model_length[index])
                b = a.cpu().numpy()
