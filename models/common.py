...
class Bottleneck_index(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, pr, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super(Bottleneck_index, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, int(c_*pr), 1, 1)
        self.cv2 = Conv(int(c_*pr), c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))

class BottleneckCSP_index(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, pr, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion   c1 128 c2 128  
        super(BottleneckCSP_index, self).__init__()
        c_ = int(c2 * e)  # hidden channels  128
        self.cv1 = Conv(int(c1 * pr), c_, 1, 1) 
        self.cv2 = nn.Conv2d(int(c1*pr), int(c_*pr), 1, 1, bias=False)
        self.cv3 = nn.Conv2d(c_, int(c_*pr), 1, 1, bias=False)
        self.cv4 = Conv(2 * int(c_*pr), int(c2*pr), 1, 1)
        self.bn = nn.BatchNorm2d(2 * int(c_*pr))  # applied to cat(cv2, cv3)
        self.act = nn.LeakyReLU(0.1, inplace=True)
        self.m = nn.Sequential(*[Bottleneck_index(c_, c_, pr, shortcut, g, e=1.0) for _ in range(n)])
